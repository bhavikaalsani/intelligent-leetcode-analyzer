...
/* ===================== ML ===================== */
async function fetchPrediction(topic, difficulty = "easy") {
  const response = await fetch(
    `${ML_SERVICE_URL}/ml/predict?topic=${encodeURIComponent(topic)}&difficulty=${encodeURIComponent(difficulty)}`
  );
...
app.get("/api/ml/predict", async (req, res) => {
  try {
    ...
    const mlData = await fetchPrediction(topic, difficulty);
    res.json(mlData);
  } catch (err) {
    console.error("ML proxy error:", err);
    res.status(500).json({ error: "ML service is not reachable" });
  }
});
...
app.get("/api/ml/predict-dynamic/:username", async (req, res) => {
  try {
    ...
    const mlData = await fetchPrediction(weakestTopic, "easy");
    ...
    console.error("Dynamic ML prediction error:", err);
