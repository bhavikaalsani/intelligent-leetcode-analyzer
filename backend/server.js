...
const ML_SERVICE_URL =
  process.env.ML_SERVICE_URL || "https://leetcode-ml-service.onrender.com";
...
app.get("/api/stats/summary", async (req, res) => {
  ...
  const accuracy = total === 0 ? 0 : ((ac / total) * 100).toFixed(2);
...
app.get("/api/ml/predict-dynamic/:username", async (req, res) => {
  try {
    ...
    const weakestTopic = stats.length ? stats[0].topic : "array";
